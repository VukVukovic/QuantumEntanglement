...
for brq=1:size(novo,2)

    rezultati = zeros(2*Nf+1,1);
    stanje=novo(1:2^Nf,brq);
    ...
    x=(-Nf:Nf)/Nf;
    %sirine(brq,1)=fwhm(x,rezultati);
    %sirine(brq,1)=std(rezultati);
    fitovanje = fit(x',rezultati,'gauss1');
    koef=coeffvalues(fitovanje);
    sirine(brq,1)=koef(3)/sqrt(2);
end

stepen=novo(2^Nf+1,:);
scatter(stepen, sirine);